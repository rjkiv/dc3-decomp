...
static CURLcode
ftp_pl_insert_finfo(struct connectdata *conn, struct curl_fileinfo *finfo) {
...
size_t Curl_ftp_parselist(char *buffer, size_t size, size_t nmemb, void *connptr) {
    ...
    if (parser->os_type == OS_TYPE_UNKNOWN && bufflen > 0) {
        ...
        parser->os_type =
            (buffer[0] >= '0' && buffer[0] <= '9') ? OS_TYPE_WIN_NT : OS_TYPE_UNIX;
    }
                ...
                case PL_WINNT_DIRORSIZE_CONTENT:
                    ...
                    if (c == ' ') {
                        ...
                        if (strcmp("<DIR>", finfo->b_data + parser->item_offset) == 0) {
                            ...
                            char *endptr;
                            finfo->size = curlx_strtoofft(
                                finfo->b_data + parser->item_offset, &endptr, 10
                            );
                            if (!*endptr) {
