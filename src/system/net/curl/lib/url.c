...
#ifdef USE_LIBSSH2
    &Curl_handler_scp,          &Curl_handler_sftp,
#endif
...
CURLcode setstropt_userpwd(char *option, char **user_storage, char **pwd_storage) {
    char *separator;
...
struct conncache *Curl_mk_connc(int type, long amount) /* set -1 to use default */
{
...
CURLcode Curl_init_userdefined(struct UserDefined *set) {
    ...
    set->redir_protocols = CURLPROTO_ALL & ~(CURLPROTO_FILE | CURLPROTO_SCP); /* not FILE
                                                                                 or SCP */
    ...
    res = setstropt(
        &set->str[STRING_SOCKS5_GSSAPI_SERVICE],
...
CURLcode Curl_setopt(struct SessionHandle *data, CURLoption option, va_list param) {
    ...
    case CURLOPT_ACCEPT_ENCODING:
        ...
        result = setstropt(
            ...
            (argptr && !*argptr) ? (char *)ALL_CONTENT_ENCODINGS : argptr
        );
        break;
        ...
        if (!argptr || data->set.postfieldsize == -1)
            ...
            if ((data->set.postfieldsize < 0)
                ...
                /* Allocate even when size == 0. This satisfies the need of possible
                   ...
                   form data.
                */
                p = malloc((size_t)(data->set.postfieldsize ? data->set.postfieldsize
                                                            : 1));
    ...
    case CURLOPT_SOCKS5_GSSAPI_SERVICE:
         ...
         */
        result = setstropt(
            &data->set.str[STRING_SOCKS5_GSSAPI_SERVICE], va_arg(param, char *)
        );
        break;
    ...
    case CURLOPT_FTPPORT:
        ...
        data->set.ftp_use_port = (NULL != data->set.str[STRING_FTPPORT]) ? TRUE : FALSE;
        break;
    ...
    case CURLOPT_TLSAUTH_USERNAME:
        result =
            setstropt(&data->set.str[STRING_TLSAUTH_USERNAME], va_arg(param, char *));
        if (data->set.str[STRING_TLSAUTH_USERNAME] && !data->set.ssl.authtype)
            data->set.ssl.authtype = CURL_TLSAUTH_SRP; /* default to SRP */
        break;
    case CURLOPT_TLSAUTH_PASSWORD:
        result =
            setstropt(&data->set.str[STRING_TLSAUTH_PASSWORD], va_arg(param, char *));
        if (data->set.str[STRING_TLSAUTH_USERNAME] && !data->set.ssl.authtype)
            data->set.ssl.authtype = CURL_TLSAUTH_SRP; /* default to SRP */
        break;
...
CURLcode Curl_disconnect(struct connectdata *conn, bool dead_connection) {
    ...
    Curl_hash_apply(data->hostcache, NULL, Curl_scan_cache_used);
#endif
...
static bool
IsPipeliningPossible(const struct SessionHandle *handle, const struct connectdata *conn) {
    if ((conn->handler->protocol & CURLPROTO_HTTP) && handle->multi
        && Curl_multi_canPipeline(handle->multi)
...
CURLcode
Curl_addHandleToPipeline(struct SessionHandle *data, struct curl_llist *pipeline) {
...
int Curl_removeHandleFromPipeline(
    struct SessionHandle *handle, struct curl_llist *pipeline
) {
    struct curl_llist_element *curr;
...
/* remove the specified connection from all (possible) pipelines and related
   queues */
void Curl_getoff_all_pipelines(struct SessionHandle *data, struct connectdata *conn) {
    bool recv_head =
        (conn->readchannel_inuse && (gethandleathead(conn->recv_pipe) == data)) ? TRUE
                                                                                : FALSE;

    bool send_head =
        (conn->writechannel_inuse && (gethandleathead(conn->send_pipe) == data)) ? TRUE
                                                                                 : FALSE;
...
static bool ConnectionExists(
    struct SessionHandle *data, struct connectdata *needle, struct connectdata **usethis
) {
    long i;
    ...
    for (i = 0; i < data->state.connc->num; i++) {
        ...
        if (canPipeline) {
            ...
            if (pipeLen > MAX_PIPELINE_LENGTH) {
                infof(
                    data,
                    "BAD! Connection #%ld has too big pipeline!\n",
                    check->connectindex
                );
            }
#endif
        ...
        if (needle->localdev || needle->localport) {
            ...
            */
            if ((check->localport != needle->localport)
                || (check->localportrange != needle->localportrange) || !check->localdev
                || !needle->localdev || strcmp(check->localdev, needle->localdev))
                continue;
        }
        ...
        if (!needle->bits.httpproxy || needle->handler->flags & PROTOPT_SSL
            ...
            if (Curl_raw_equal(needle->handler->scheme, check->handler->scheme)
                ...
                if (needle->handler->flags & PROTOPT_SSL) {
                    ...
                    if (!Curl_ssl_config_matches(
                            &needle->ssl_config, &check->ssl_config
                        )) {
                        DEBUGF(infof(
                            data,
            ...
            if (check->bits.proxy && (needle->proxytype == check->proxytype)
                && (needle->bits.tunnel_proxy == check->bits.tunnel_proxy)
                && Curl_raw_equal(needle->proxy.name, check->proxy.name)
                && needle->port == check->port) {
...
static void ConnectionDone(struct connectdata *conn) { conn->inuse = FALSE; }
 ...
 * this call.
 */
static void ConnectionStore(struct SessionHandle *data, struct connectdata *conn) {
    long i;
    for (i = 0; i < data->state.connc->num; i++) {
    ...
    if (i == data->state.connc->num) {
        ...
        if (-1 != i)
            infof(
                data,
                "Connection (#%ld) was killed to make room (holds %ld)\n",
                i,
                data->state.connc->num
            );
        else
...
CURLcode Curl_connected_proxy(struct connectdata *conn) {
    ...
    case CURLPROXY_SOCKS4:
        return Curl_SOCKS4(
            conn->proxyuser, conn->host.name, conn->remote_port, FIRSTSOCKET, conn, FALSE
        );

    case CURLPROXY_SOCKS4A:
        return Curl_SOCKS4(
            conn->proxyuser, conn->host.name, conn->remote_port, FIRSTSOCKET, conn, TRUE
        );
...
static CURLcode
ConnectPlease(struct SessionHandle *data, struct connectdata *conn, bool *connected) {
    CURLcode result;
    Curl_addrinfo *addr;
     ...
     *************************************************************/
    result = Curl_connecthost(
        conn, conn->dns_entry, &conn->sock[FIRSTSOCKET], &addr, connected
    );
    if (CURLE_OK == result) {
 ...
 */
static bool tld_check_name(struct SessionHandle *data, const char *ace_hostname) {
    size_t err_pos;
    char *uc_name = NULL;
    int rc;
#ifndef CURL_DISABLE_VERBOSE_STRINGS
 ...
 */
static void
fix_hostname(struct SessionHandle *data, struct connectdata *conn, struct hostname *host) {
#ifndef USE_LIBIDN
    (void)data;
    (void)conn;
...
static struct connectdata *allocate_conn(struct SessionHandle *data) {
    ...
    if (data->multi && Curl_multi_canPipeline(data->multi) && !conn->master_buffer) {
    ...
    if (!conn->send_pipe || !conn->recv_pipe || !conn->pend_pipe || !conn->done_pipe)
        goto error;
...
static CURLcode
findprotocol(struct SessionHandle *data, struct connectdata *conn, const char *protostr) {
 ...
 */
static CURLcode
parse_proxy(struct SessionHandle *data, struct connectdata *conn, char *proxy) {
    char *prox_portno;
    char *endofprot;
...
static CURLcode parse_proxy_auth(struct SessionHandle *data, struct connectdata *conn) {
    ...
    if (data->set.str[STRING_PROXYPASSWORD] != NULL) {
        strncpy(
            proxypasswd, data->set.str[STRING_PROXYPASSWORD], MAX_CURL_PASSWORD_LENGTH
        );
 ...
 */
static CURLcode parse_url_userpass(
    struct SessionHandle *data, struct connectdata *conn, char *user, char *passwd
) {
 ...
 *************************************************************/
static CURLcode parse_remote_port(struct SessionHandle *data, struct connectdata *conn) {
    char *portptr;
    char endbracket;
    ...
    /* Note that at this point, the IPv6 address cannot contain any scope
       ...
       function */
    if ((1 == sscanf(conn->host.name, "[%*45[0123456789abcdefABCDEF:.]%c", &endbracket))
        && (']' == endbracket)) {
        ...
        if (conn->bits.httpproxy) {
            ...
            if (conn->bits.type_set)
                snprintf(
                    ...
                    sizeof(type),
                    ";type=%c",
                    data->set.prefer_ascii ? 'A' : (data->set.ftp_list_only ? 'D' : 'I')
                );
...
static void override_userpass(
    ...
    if (data->set.use_netrc != CURL_NETRC_IGNORED) {
        if (Curl_parsenetrc(
                conn->host.name, user, passwd, data->set.str[STRING_NETRC_FILE]
            )) {
            infof(
                data,
                "Couldn't find host %s in the " DOT_CHAR "netrc file; using defaults\n",
                conn->host.name
            );
        } else {
 ...
 */
static CURLcode
set_userpass(struct connectdata *conn, const char *user, const char *passwd) {
 ...
 *************************************************************/
static CURLcode
resolve_server(struct SessionHandle *data, struct connectdata *conn, bool *async) {
    CURLcode result = CURLE_OK;
        ...
        *async = FALSE;
        ...
        if (!conn->proxy.name || !*conn->proxy.name) {
            ...
            rc = Curl_resolv_timeout(
                conn, conn->host.name, (int)conn->port, &hostaddr, timeout_ms
            );
            ...
            /* resolve proxy */
            rc = Curl_resolv_timeout(
                conn, conn->proxy.name, (int)conn->port, &hostaddr, timeout_ms
            );
...
static CURLcode
create_conn(struct SessionHandle *data, struct connectdata **in_connect, bool *async) {
    CURLcode result = CURLE_OK;
    struct connectdata *conn;
...
CURLcode Curl_done(
    ...
    if ((conn->send_pipe->size + conn->recv_pipe->size != 0 && !data->set.reuse_forbid
         && !conn->bits.close))
