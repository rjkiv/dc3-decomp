...
static void state(struct connectdata *conn, pop3state newstate) {
    ...
    static const char *const names[] = {
        "STOP", "SERVERGREET", "USER", "PASS", "STARTTLS",
        "LIST", "LIST_SINGLE", "RETR", "QUIT",
        /* LAST */
    };
#endif
...
static CURLcode pop3_state_user(struct connectdata *conn) {
    ...
    /* send USER */
    result =
        Curl_pp_sendf(&conn->proto.pop3c.pp, "USER %s", pop3->user ? pop3->user : "");
    if (result)
        return result;
...
static CURLcode
pop3_state_starttls_resp(struct connectdata *conn, int pop3code, pop3state instate) {
...
/* for USER responses */
static CURLcode
pop3_state_user_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    CURLcode result = CURLE_OK;
    struct SessionHandle *data = conn->data;
    ...
    if (pop3code != 'O') {
        ...
        result = Curl_pp_sendf(
            &conn->proto.pop3c.pp, "PASS %s", pop3->passwd ? pop3->passwd : ""
        );
    if (result)
        return result;
...
/* for PASS responses */
static CURLcode
pop3_state_pass_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    CURLcode result = CURLE_OK;
    struct SessionHandle *data = conn->data;
    (void)instate; /* no use for this yet */
...
static CURLcode
pop3_state_retr_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    CURLcode result = CURLE_OK;
    struct SessionHandle *data = conn->data;
    struct FTP *pop3 = data->state.proto.pop3;
    struct pop3_conn *pop3c = &conn->proto.pop3c;
    ...
    /* POP3 download */
    Curl_setup_transfer(conn, FIRSTSOCKET, -1, FALSE, pop3->bytecountp, -1, NULL); /* no
                                                                                      upload
                                                                                      here
                                                                                    */
...
static CURLcode
pop3_state_list_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    CURLcode result = CURLE_OK;
    struct SessionHandle *data = conn->data;
    struct FTP *pop3 = data->state.proto.pop3;
    struct pop3_conn *pop3c = &conn->proto.pop3c;
...
static CURLcode
pop3_state_list_single_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    CURLcode result = CURLE_OK;
    struct SessionHandle *data = conn->data;
    (void)instate; /* no use for this yet */
 ...
 */
static CURLcode pop3_connect(struct connectdata *conn, bool *done) /* see description
                                                                      above */
{
    CURLcode result;
    struct pop3_conn *pop3c = &conn->proto.pop3c;
...
CURLcode Curl_pop3_write(struct connectdata *conn, char *str, size_t nread) {
    ...
    if (!memcmp(POP3_EOB, &str[nread - check], check)) {
        ...
        /* not a match, but we matched a piece before so we must now
           ...
           this buffer */
        result = Curl_client_write(conn, CLIENTWRITE_BODY, (char *)POP3_EOB, pop3c->eob);
        if (result)
            return result;
        pop3c->eob = 0;
    }
