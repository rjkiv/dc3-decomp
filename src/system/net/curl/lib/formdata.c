    ...
    if (filename) { /* in case a NULL was passed in */
        for (i = 0; i < sizeof(ctts) / sizeof(ctts[0]); i++) {
            if (strlen(filename) >= strlen(ctts[i].extension)) {
                if (strequal(
                        filename + strlen(filename) - strlen(ctts[i].extension),
                        ctts[i].extension
                    )) {
...
static CURLcode
AddFormDataf(struct FormData **formp, curl_off_t *size, const char *fmt, ...) {
    char s[4096];
    va_list ap;
    va_start(ap, fmt);
    ...
    for (ptr = data; ptr; ptr = ptr->next) {
        if ((ptr->type == FORM_FILE) || (ptr->type == FORM_CALLBACK)) {
            ...
            do {
                ...
                if ((nread == (size_t)-1) || (nread > sizeof(buffer))
...
CURLcode Curl_getformdata(
    ...
    *finalform = NULL; /* default form is empty */
    ...
    result = AddFormDataf(
        ...
        NULL,
        "%s; boundary=%s\r\n",
        custom_content_type ? custom_content_type : "Content-Type: multipart/form-data",
        boundary
    );
    ...
    do {
        ...
        /* Maybe later this should be disabled when a custom_content_type is
           ...
           types.
        */
        result = AddFormDataf(&form, &size, "Content-Disposition: form-data; name=\"");
        if (result)
            break;
        ...
        do {
            ...
            if (post->more) {
                ...
                result = AddFormDataf(
                    ...
                    fileboundary,
                    (file->showfilename ? file->showfilename : filebasename)
                );
                if (filebasename)
                    free(filebasename);
                if (result)
                ...
                if (post->showfilename || (post->flags & HTTPPOST_FILENAME)) {
                    ...
                    result = AddFormDataf(
                        ...
                        &size,
                        "; filename=\"%s\"",
                        (post->showfilename ? post->showfilename : filebasename)
                    );
                    if (filebasename)
                        free(filebasename);
                }
            ...
            if ((post->flags & HTTPPOST_FILENAME) || (post->flags & HTTPPOST_READFILE)) {
                ...
                fileread = strequal("-", file->contents)
                    ? stdin
                    : fopen(file->contents, "rb"); /* binary read for win32  */
                ...
                if (fileread) {
                    if (fileread != stdin) {
                        ...
                        while ((nread = fread(buffer, 1, sizeof(buffer), fileread))
                               != 0) {
                            result =
                                AddFormData(&form, FORM_CONTENT, buffer, nread, &size);
                            if (result)
                                break;
                        }
                    }
                ...
                result = AddFormData(
                    &form, FORM_CONTENT, post->buffer, post->bufferlength, &size
                );
            else if (post->flags & HTTPPOST_CALLBACK)
                /* the contents should be read with the callback and the size
                   is set with the contentslength */
                result = AddFormData(
                    &form, FORM_CALLBACK, post->userp, post->contentslength, &size
                );
            else
                /* include the contents we got */
                result = AddFormData(
                    &form, FORM_CONTENT, post->contents, post->contentslength, &size
                );
