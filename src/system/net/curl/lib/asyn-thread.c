 ...
 * structure).  Does nothing here.
 */
void Curl_resolver_cleanup(void *resolver) { (void)resolver; }
 ...
 */
void Curl_resolver_cancel(struct connectdata *conn) { destroy_async_data(&conn->async); }
...
static const char *gai_strerror(int ecode) {
    ...
    return "The service passed was not recognized for the "
           "specified socket type" case EAI_SOCKTYPE:
    return "The intended socket type was not recognized" case EAI_SYSTEM:
...
#define HAVE_GAI_STRERROR
     ...
     */
    CURLcode Curl_resolver_wait_resolv(
    ...
    int Curl_resolver_getsock(
        struct connectdata * conn, curl_socket_t * socks, int numsocks
    ) {
