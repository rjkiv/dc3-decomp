...
#define SOCKS4REQLEN 262
    ...
    if (!protocol4a) {
        ...
        if (hp) {
            ...
            if (4 == sscanf(buf, "%hu.%hu.%hu.%hu", &ip[0], &ip[1], &ip[2], &ip[3])) {
                /* Set DSTIP */
     ...
     */
    {
        ...
        int packetsize = 9 + (int)strlen((char *)socksreq + 8); /* size including NUL */
        ...
        code = Curl_write_plain(
            conn, sock, (char *)socksreq, packetsize + hostnamelen, &written
        );
        ...
        /* Receive response */
        result =
            Curl_blockread_all(conn, sock, (char *)socksreq, packetsize, &actualread);
        ...
        /* wrong version ? */
        if (socksreq[0] != 0) {
            failf(data, "SOCKS4 reply has wrong version, version should be 4.");
            return CURLE_COULDNT_CONNECT;
        }
...
CURLcode Curl_SOCKS5(
    ...
    */
    ...
    code =
        Curl_write_plain(conn, sock, (char *)socksreq, (2 + (int)socksreq[1]), &written);
    ...
    if (!socks5_resolve_local) {
        ...
        if (hp) {
            ...
            if (sscanf(buf, "%hu.%hu.%hu.%hu", &ip[0], &ip[1], &ip[2], &ip[3]) == 4) {
                socksreq[4] = (unsigned char)ip[0];
                socksreq[5] = (unsigned char)ip[1];
                socksreq[6] = (unsigned char)ip[2];
                socksreq[7] = (unsigned char)ip[3];
    ...
    if (!conn->socks5_gssapi_enctype) {
        ...
        if (packetsize > 10) {
            ...
            result = Curl_blockread_all(
                conn, sock, (char *)&socksreq[10], packetsize, &actualread
            );
            if ((result != CURLE_OK) || (actualread != packetsize)) {
                failf(data, "Failed to receive SOCKS5 connect request ack.");
                return CURLE_COULDNT_CONNECT;
