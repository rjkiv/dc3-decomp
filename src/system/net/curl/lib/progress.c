...
static void time2str(char *r, curl_off_t seconds) {
    ...
    if (h <= CURL_OFF_T_C(99)) {
        ...
        snprintf(r, 9, "%2" FORMAT_OFF_T ":%02" FORMAT_OFF_T ":%02" FORMAT_OFF_T, h, m, s);
    } else {
...
#define ONE_PETABYTE (CURL_OFF_T_C(1024) * ONE_TERABYTE)
    ...
    if (bytes < CURL_OFF_T_C(100000))
        ...
        snprintf(
            max5,
            6,
            "%2" FORMAT_OFF_T ".%0" FORMAT_OFF_T "M",
            bytes / ONE_MEGABYTE,
    ...
    // #if (CURL_SIZEOF_CURL_OFF_T > 4)
        ...
        snprintf(
            max5,
            6,
            "%2" FORMAT_OFF_T ".%0" FORMAT_OFF_T "G",
            bytes / ONE_GIGABYTE,
    ...
    // else snprintf(max5, 6, "%4" FORMAT_OFF_T "M", bytes / ONE_MEGABYTE);
...
void Curl_pgrsTime(struct SessionHandle *data, timerid timer) {
    ...
    case TIMER_POSTRANSFER:
        ...
        break;
    case TIMER_REDIRECT:
        data->progress.t_redirect = Curl_tvdiff_secs(Curl_tvnow(), data->progress.start);
        break;
    }
}
...
int Curl_pgrsUpdate(struct connectdata *conn) {
    ...
    data->progress.timespent = (double)(now.tv_sec - data->progress.start.tv_sec)
        + (double)(now.tv_usec - data->progress.start.tv_usec) / 1000000.0;
    timespent = (curl_off_t)data->progress.timespent;

    /* The average download speed this far */
    data->progress.dlspeed =
        (curl_off_t)((double)data->progress.downloaded
                     / (data->progress.timespent > 0 ? data->progress.timespent : 1));

    /* The average upload speed this far */
    data->progress.ulspeed =
        (curl_off_t)((double)data->progress.uploaded
                     / (data->progress.timespent > 0 ? data->progress.timespent : 1));
    ...
    if (data->progress.lastshow != (long)now.tv_sec) {
        ...
        countindex = ((data->progress.speeder_c >= CURR_TIME) ? CURR_TIME
                                                              : data->progress.speeder_c)
            - 1;
        ...
        if (countindex) {
            ...
            span_ms = Curl_tvdiff(now, data->progress.speeder_time[checkindex]);
            if (0 == span_ms)
