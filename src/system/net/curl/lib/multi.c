...
static void
multi_connc_remove_handle(struct Curl_multi *multi, struct SessionHandle *data);
static void singlesocket(struct Curl_multi *multi, struct Curl_one_easy *easy);
static CURLMcode add_closure(struct Curl_multi *multi, struct SessionHandle *data);
static int update_timer(struct Curl_multi *multi);

static CURLcode
addHandleToSendOrPendPipeline(struct SessionHandle *handle, struct connectdata *conn);
static int checkPendPipeline(struct connectdata *conn);
static void
moveHandleFromSendToRecvPipeline(struct SessionHandle *handle, struct connectdata *conn);
static void
moveHandleFromRecvToDonePipeline(struct SessionHandle *handle, struct connectdata *conn);
static bool isHandleAtHead(struct SessionHandle *handle, struct curl_llist *pipeline);
static CURLMcode
add_next_timeout(struct timeval now, struct Curl_multi *multi, struct SessionHandle *d);

#ifdef DEBUGBUILD
static const char *const statename[] = {
    "INIT",         "CONNECT",     "WAITRESOLVE", "WAITCONNECT", "WAITPROXYCONNECT",
    "PROTOCONNECT", "WAITDO",      "DO",          "DOING",       "DO_MORE",
    "DO_DONE",      "WAITPERFORM", "PERFORM",     "TOOFAST",     "DONE",
    "COMPLETED",    "MSGSENT",
};
#endif
...
#ifdef DEBUGBUILD
    if (easy->easy_conn) {
        if (easy->state > CURLM_STATE_CONNECT && easy->state < CURLM_STATE_COMPLETED)
...
sh_addentry(struct curl_hash *sh, curl_socket_t s, struct SessionHandle *data) {
    struct Curl_sh_entry *there = Curl_hash_pick(sh, (char *)&s, sizeof(curl_socket_t));
...
static struct curl_hash *sh_init(void) {
    return Curl_hash_alloc(
        CURL_SOCKET_HASH_TABLE_SIZE, hash_fd, fd_key_compare, sh_freeentry
    );
}
...
CURLMcode curl_multi_remove_handle(CURLM *multi_handle, CURL *curl_handle) {
    ...
    if (easy) {
        ...
        bool easy_owns_conn =
            (easy->easy_conn && (easy->easy_conn->data == easy->easy_handle)) ? TRUE
                                                                              : FALSE;
        ...
        if (easy->easy_conn
            && (easy->easy_conn->send_pipe->size + easy->easy_conn->recv_pipe->size > 1)
            && easy->state > CURLM_STATE_WAITDO && easy->state < CURLM_STATE_COMPLETED) {
        ...
        if (easy->easy_handle->state.connc->type == CONNCACHE_MULTI) {
            ...
            */
            if (easy->easy_conn && easy_owns_conn
                && (easy->easy_conn->send_pipe->size + easy->easy_conn->recv_pipe->size
                    == 0))
...
static int
waitconnect_getsock(struct connectdata *conn, curl_socket_t *sock, int numsocks) {
    if (!numsocks)
...
static CURLMcode
multi_runsingle(struct Curl_multi *multi, struct timeval now, struct Curl_one_easy *easy) {
    ...
    do {
        ...
        if (easy->easy_conn && (easy->state >= CURLM_STATE_CONNECT)
            ...
            timeout_ms = Curl_timeleft(
                data, &now, (easy->state <= CURLM_STATE_WAITDO) ? TRUE : FALSE
            );
            ...
            if (timeout_ms < 0) {
                ...
                if (easy->state == CURLM_STATE_WAITRESOLVE)
                    ...
                    failf(
                        data,
                        "Operation timed out after %ld milliseconds with %" FORMAT_OFF_T
        ...
        case CURLM_STATE_CONNECT:
            ...
            easy->result =
                Curl_connect(data, &easy->easy_conn, &async, &protocol_connect);

            if (CURLE_OK == easy->result) {
                /* Add this handle to the send or pend pipeline */
                easy->result = addHandleToSendOrPendPipeline(data, easy->easy_conn);
                if (CURLE_OK != easy->result)
                    ...
                    if (async)
                        ...
                        if (protocol_connect)
                            multistate(
                                easy,
                                multi->pipelining_enabled ? CURLM_STATE_WAITDO
                                                          : CURLM_STATE_DO
                            );
                        else {
        ...
        case CURLM_STATE_WAITRESOLVE:
            /* awaiting an asynch name resolve to complete */
            {
                ...
                if (dns) {
                    ...
                    easy->result =
                        Curl_async_resolved(easy->easy_conn, &protocol_connect);
        ...
        case CURLM_STATE_WAITCONNECT:
            ...
            easy->result = Curl_is_connected(easy->easy_conn, FIRSTSOCKET, &connected);
            if (connected) {
                if (!easy->result)
                    /* if everything is still fine we do the protocol-specific connect
                       setup */
                    easy->result =
                        Curl_protocol_connect(easy->easy_conn, &protocol_connect);
            }
            ...
            if (connected) {
                if (!protocol_connect) {
                    ...
                    */
                    ...
                    multistate(
                        easy,
                        multi->pipelining_enabled ? CURLM_STATE_WAITDO : CURLM_STATE_DO
                    );
        ...
        case CURLM_STATE_PROTOCONNECT:
            ...
            easy->result = Curl_protocol_connecting(easy->easy_conn, &protocol_connect);
            if ((easy->result == CURLE_OK) && protocol_connect) {
                /* after the connect has completed, go WAITDO or DO */
                multistate(
                    easy, multi->pipelining_enabled ? CURLM_STATE_WAITDO : CURLM_STATE_DO
                );
                result = CURLM_CALL_MULTI_PERFORM;
            } else if (easy->result) {
                /* failure detected */
        ...
        case CURLM_STATE_WAITDO:
            ...
            infof(
                data,
                "Conn %ld send pipe %zu inuse %d athead %d\n",
                easy->easy_conn->connectindex,
                easy->easy_conn->send_pipe->size,
                easy->easy_conn->writechannel_inuse ? 1 : 0,
                isHandleAtHead(data, easy->easy_conn->send_pipe) ? 1 : 0
            );
#endif
            if (!easy->easy_conn->writechannel_inuse
                && isHandleAtHead(data, easy->easy_conn->send_pipe)) {
                /* Grab the channel */
        ...
        case CURLM_STATE_DOING:
            ...
            if (CURLE_OK == easy->result) {
                if (dophase_done) {
                    ...
                    multistate(
                        easy,
                        easy->easy_conn->bits.do_more ? CURLM_STATE_DO_MORE
                                                      : CURLM_STATE_DO_DONE
                    );
                    result = CURLM_CALL_MULTI_PERFORM;
                } /* dophase_done */
        ...
        case CURLM_STATE_DO_MORE:
            /* Ready to do more? */
            easy->result =
                Curl_is_connected(easy->easy_conn, SECONDARYSOCKET, &connected);
            if (connected) {
                /*
        ...
        case CURLM_STATE_WAITPERFORM:
            ...
            if (!easy->easy_conn->readchannel_inuse
                && isHandleAtHead(data, easy->easy_conn->recv_pipe)) {
                /* Grab the channel */
                ...
                infof(
                    data,
                    "Conn %ld recv pipe %zu inuse %d athead %d\n",
                    easy->easy_conn->connectindex,
                    easy->easy_conn->recv_pipe->size,
                    easy->easy_conn->readchannel_inuse ? 1 : 0,
                    isHandleAtHead(data, easy->easy_conn->recv_pipe) ? 1 : 0
                );
            }
#endif
            break;
        ...
        case CURLM_STATE_PERFORM:
            ...
            if ((data->set.max_send_speed > 0)
                ...
                buffersize =
                    (int)(data->set.buffer_size ? data->set.buffer_size : BUFSIZE);
                timeout_ms = Curl_sleep_time(
                    data->set.max_send_speed, data->progress.ulspeed, buffersize
                );
            ...
            if ((data->set.max_recv_speed > 0)
                ...
                buffersize =
                    (int)(data->set.buffer_size ? data->set.buffer_size : BUFSIZE);
                timeout_ms = Curl_sleep_time(
                    data->set.max_recv_speed, data->progress.dlspeed, buffersize
                );
                Curl_expire(data, timeout_ms);
                break;
            }
            ...
            if (easy->result) {
                ...
                moveHandleFromRecvToDonePipeline(data, easy->easy_conn);
        ...
        case CURLM_STATE_DONE:
            ...
            if (easy->easy_conn) {
                ...
                Curl_removeHandleFromPipeline(data, easy->easy_conn->done_pipe);
                /* Check if we can move pending requests to send pipe */
                checkPendPipeline(easy->easy_conn);
        ...
        if (easy->state < CURLM_STATE_COMPLETED) {
            if (CURLE_OK != easy->result) {
                ...
                if (easy->easy_conn) {
                    ...
                    easy->easy_conn->writechannel_inuse = FALSE;
                    easy->easy_conn->readchannel_inuse = FALSE;
                    Curl_removeHandleFromPipeline(data, easy->easy_conn->send_pipe);
                    Curl_removeHandleFromPipeline(data, easy->easy_conn->recv_pipe);
                    Curl_removeHandleFromPipeline(data, easy->easy_conn->done_pipe);
                    /* Check if we can move pending requests to send pipe */
                    checkPendPipeline(easy->easy_conn);
            ...
            /* if there's still a connection to use, call the progress function */
                ...
                multistate(
                    easy,
                    (easy->state < CURLM_STATE_DONE) ? CURLM_STATE_DONE
...
static void singlesocket(struct Curl_multi *multi, struct Curl_one_easy *easy) {
    ...
    for (i = 0; (i < MAX_SOCKSPEREASYHANDLE)
        ...
        multi->socket_cb(
            easy->easy_handle, s, action, multi->socket_userp, entry->socketp
        );
...
CURLMcode curl_multi_socket(CURLM *multi_handle, curl_socket_t s, int *running_handles) {
    CURLMcode result =
        multi_socket((struct Curl_multi *)multi_handle, FALSE, s, 0, running_handles);
    if (CURLM_OK >= result)
...
CURLMcode curl_multi_socket_action(
    ...
    CURLMcode result = multi_socket(
        (struct Curl_multi *)multi_handle, FALSE, s, ev_bitmask, running_handles
    );
    if (CURLM_OK >= result)
        update_timer((struct Curl_multi *)multi_handle);
    return result;
...
{
    CURLMcode result = multi_socket(
        (struct Curl_multi *)multi_handle, TRUE, CURL_SOCKET_BAD, 0, running_handles
    );
    if (CURLM_OK >= result)
        update_timer((struct Curl_multi *)multi_handle);
    return result;
...
addHandleToSendOrPendPipeline(struct SessionHandle *handle, struct connectdata *conn) {
    ...
    if (!Curl_isPipeliningEnabled(handle) || pipeLen == 0)
        ...
        if (conn->server_supports_pipelining && pipeLen < MAX_PIPELINE_LENGTH)
            pipeline = conn->send_pipe;
        else
...
static int checkPendPipeline(struct connectdata *conn) {
    ...
    if (conn->server_supports_pipelining || pipeLen == 0) {
        ...
        while (pipeLen < maxPipeLen && curr) {
            Curl_llist_move(conn->pend_pipe, curr, conn->send_pipe, conn->send_pipe->tail);
...
moveHandleFromSendToRecvPipeline(struct SessionHandle *handle, struct connectdata *conn) {
    ...
    while (curr) {
        if (curr->ptr == handle) {
            Curl_llist_move(conn->send_pipe, curr, conn->recv_pipe, conn->recv_pipe->tail);
...
#ifdef DEBUGBUILD
                infof(
                    conn->data, "%p is at send pipe head B!\n", conn->send_pipe->head->ptr
                );
#endif
                Curl_expire(conn->send_pipe->head->ptr, 1);
            }
...
moveHandleFromRecvToDonePipeline(struct SessionHandle *handle, struct connectdata *conn) {
    ...
    while (curr) {
        if (curr->ptr == handle) {
            Curl_llist_move(conn->recv_pipe, curr, conn->done_pipe, conn->done_pipe->tail);
            break;
        }
        curr = curr->next;
    }
...
void Curl_expire(struct SessionHandle *data, long milli) {
    ...
    if (!milli) {
        ...
        if (nowp->tv_sec || nowp->tv_usec) {
            ...
            rc = Curl_splayremovebyaddr(
                multi->timetree, &data->state.timenode, &multi->timetree
            );
            if (rc)
...
void Curl_multi_dump(const struct Curl_multi *multi_handle) {
    ...
    int i;
    fprintf(
        stderr, "* Multi status: %d handles, %d alive\n", multi->num_easy, multi->num_alive
    );
