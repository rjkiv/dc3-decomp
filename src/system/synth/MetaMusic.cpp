...
MetaMusic::MetaMusic(HxMaster *, const char *cc)
...
void MetaMusic::Start() {
    if (!mPlayFromBuffer || mBuf) {
        if (mStream && mStream->IsPlaying()) {
            ...
            if (mLoop) {
                mStream->SetJump(0, 0, 0);
            }
            if (unk88) {
                LoadStreamFx();
                for (int i = 0; i < 6; i++) {
                    mStream->SetFXSend(i, mShellFx->Find<FxSendEQ>("eq.send", true));
                }
            }
            unk78 = true;
        }
            ...
            for (int i = 0; i < numChannels; i++) {
                ...
                if (chanArr7c && chanArr80) {
                    for (ObjDirItr<FxSend> it(mShellFx, true); it != nullptr; ++it) {
                        ...
                        MILO_ASSERT(thisFxConfigPre->Size() ==
                        thisFxConfigPost->Size(), 0x18E);
...
void MetaMusic::LoadStreamFx() {
    MILO_ASSERT(mShellFx, 536);
    FilePath fp(".", unk88);
    mShellFx.LoadFile(fp, true, false, kLoadFront, false);
