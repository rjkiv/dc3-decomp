...
static uint decodeoptions(const char *), optbit(char);
...
static void printresults(const char *const[]), printcmdline(const char *const[]),
    printfilter(), printgain(const char *, complex);
...
global int main(int argc, const char *const argv[]) {
    ...
    checkoptions();
    setdefaults();
    if (options & opt_re) { /* bandpass resonator */
        if (options & opt_bp)
            compute_bpres();
        /* bandstop resonator (notch) */
        if (options & opt_bs)
            compute_notch();
        /* allpass resonator */
        if (options & opt_ap)
...
static void readcmdline(const char *const argv[]) {
    ...
    until(argv[ap] == NULL) {
        ...
        if (m & opt_ch)
            chebrip = getfarg(argv[ap++]);
        if (m & opt_a) {
            raw_alpha1 = getfarg(argv[ap++]);
            raw_alpha2 = (argv[ap] != NULL && argv[ap][0] != '-') ? getfarg(argv[ap++])
                                                                  : raw_alpha1;
        }
        if (m & opt_Z)
        ...
        if (m & opt_p) {
            while (argv[ap] != NULL && argv[ap][0] >= '0' && argv[ap][0] <= '9') {
                int p = atoi(argv[ap++]);
                /* out-of-range value will be picked up later */
                if (p < 0 || p > 31)
                    p = 31;
                polemask |= (1 << p);
            }
...
static uint decodeoptions(const char *s) {
    unless(*(s++) == '-') usage();
    uint m = 0;
    if (seq(s, "Be"))
        m |= opt_be;
...
static void usage() {
    ...
    fprintf(
        stderr,
        "Usage: mkfilter [-Be | -Bu | -Ch <r> | -Pi] [-Lp | -Hp | -Bp | -Bs] [-p <n1> <n2> ...] [-{lwz}] "
        "[-Z <alphaz>] "
    ...
    fprintf(
        stderr, "  -Lp, Hp, Bp, Bs, Ap = lowpass, highpass, bandpass, bandstop, allpass\n"
    );
...
static void checkoptions() {
    ...
    if (options & opt_re) {
        ...
        if (options & opt_o) {
            ...
            if (options & opt_p) { /* "order" bits set */
                uint m = (1 << order) - 1;
                if ((polemask & ~m) != 0)
                    opterror(
                        "order=%d, so args to -p must be in range 0 .. %d",
                        order,
                        order - 1
                    );
            }
...
static void setdefaults() { /* use all poles */
    unless(options & opt_p) polemask = ~0;
...
static void compute_s() {
    ...
    if (options & opt_ch) { /* modify for Chebyshev (p. 136 DeFatta et al.) */
        if (chebrip >= 0.0) {
            fprintf(
                stderr, "mkfilter: Chebyshev ripple is %g dB; must be .lt. 0.0\n", chebrip
            );
            exit(1);
        }
...
static void compute_bpres() {
    ...
    if (infq) { /* oscillator */
        ...
        for (int i = 0; i < 50 && !cvg; i++) {
            ...
            complex g = evaluate(
                topcoeffs, zplane.numzeros, botcoeffs, zplane.numpoles, expj(theta)
            );
            /* approx to atan2 */
...
static void printgain(const char *str, complex gain) {
    ...
    if (r > EPS)
        printf("   phase = %14.10f pi", atan2(gain) / PI);
    putchar('\n');
}
...
/* given (real) Z-plane poles & zeros, compute & print recurrence relation */
...
static void prcomplex(complex z) { printf("%14.10f + j %14.10f", z.re, z.im); }
