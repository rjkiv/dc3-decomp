...
RndMat::RndMat()
    : mMetaMaterial(this), mFlags(kColorModNone), mToggleDisplayAllProps(0), unk225(0),
      unk226(0), mDirty(3) {
...
BEGIN_PROPSYNCS(RndMat)
    ...
    SYNC_MAT_PROP(alpha_write, mAlphaWrite, 2) {
        static Symbol _s("force_alpha_write");
        if (sym == _s)
            return PropSync(mForceAlphaWrite, _val, _prop, _i + 1, _op) != false;
    }

    {
        static Symbol _s("next_pass");
        if (sym == _s) {
            Symbol action("next_pass_edit_action");
            if (!(_op & (kPropSize | kPropGet)) && !IsEditable(action)) {
                return true;
            }
            return PropSync(mNextPass, _val, _prop, _i + 1, _op) != false;
        }
    }
...
void RndMat::UpdatePropertiesFromMetaMat() {
    if (mMetaMaterial) {
        unk226 = true;
        String propStr;
        std::list<Symbol> matProps;
        ListProperties(matProps, "Mat", 0, nullptr, false);
        FOREACH (it, matProps) {
            static Symbol metamaterial("metamaterial");
            Symbol cur = *it;
            if (cur != metamaterial) {
                Symbol s = cur == "alpha_threshold" ? "alpha_cut" : cur;
                MatPropEditAction a = GetMetaMatPropAction(s);
                if ((a == kPropDefault || a == kPropForce)
                    && PropValDifferent(cur, mMetaMaterial)) {
                    if (cur == "tex_xfm") {
                        SetTexXfm(mMetaMaterial->TexXfm());
                    } else {
                        SetProperty(cur, *mMetaMaterial->Property(cur));
                    }
                    AddOverridePropName(propStr, cur);
                }
            }
        }
        if (!propStr.empty()) {
